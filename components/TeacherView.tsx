...
import { Upload, Users, FileText, AlertTriangle, Loader2, Edit2, Save, X, Search, Activity, CalendarCheck, BarChart3, Check, Sparkles } from 'lucide-react';

declare const XLSX: any;

interface TeacherViewProps {
  students: Student[];
  onImportData: (newStudents: Student[]) => Promise<void>;
  onUpdateStudentReport: (mhs: string, report: AIReport, actions: StudyAction[]) => void;
  onLogout: () => void;
}

const TeacherView: React.FC<TeacherViewProps> = ({ students, onImportData, onUpdateStudentReport, onLogout }) => {
      ...
      wb.SheetNames.forEach((sheetName: string) => {
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          if (existingScoreIndex >= 0) {
            student.scores[existingScoreIndex] = scoreEntry;
          } else {
            student.scores.push(scoreEntry);
          }
        }
      });

      const newStudentList = Array.from(studentMap.values());
      await onImportData(newStudentList);
  ...
  const handleGenerateAI = async (student: Student) => {
    ...
    try {
      ...
      alert("Không thể tạo báo cáo (kiểm tra GEMINI_API_KEY trên Vercel).");
    } finally {
      setLoadingMhs(null);
    }
  };
  ...
  const handleBulkGenerate = async () => {
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        try {
            // Check if API key is present before calling to avoid generic error
            ...
            // Update state immediately for this student
            onUpdateStudentReport(student.mhs, report, newActions);
    ...
    <div className="min-h-screen bg-[#f8fafc] flex flex-col font-sans relative">
      ...
      {bulkProgress && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center">
            <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-md w-full text-center animate-in fade-in zoom-in duration-300">
                ...
                <p className="text-slate-500 mb-6">Đang xử lý: <span className="font-bold text-indigo-600">{bulkProgress.currentName}</span></p>
      ...
      <header className="bg-white/90 backdrop-blur-sm border-b border-slate-200/60 px-8 py-5 flex justify-between items-center sticky top-0 z-20 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onLogout} className="text-sm font-semibold text-slate-500 hover:text-red-500 transition-colors px-2 ml-2">Đăng xuất</button>
        </div>
      </header>
          ...
          <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[90vh] overflow-hidden shadow-2xl flex flex-col transform transition-transform duration-300 scale-100">
            ...
            <div className="flex justify-between items-center p-6 border-b border-slate-100 bg-white z-10">
              <div>
                 ...
                 <p className="text-sm text-slate-500 mt-1">MHS: <span className="font-mono text-indigo-600">{viewingStudent.mhs}</span> | Lớp: {viewingStudent.class}</p>
              </div>
            ...
            <div className="flex border-b border-slate-100 px-6">
                <button 
                    ...
                    className={`py-3 px-4 text-sm font-semibold border-b-2 transition-colors ${activeTab === 'report' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                ...
                <button 
                    ...
                    className={`py-3 px-4 text-sm font-semibold border-b-2 transition-colors ${activeTab === 'tracking' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
                >
                    <div className="flex items-center gap-2">
                ...
                viewingStudent.aiReport ? (
                    <>
                        ...
                        <div className="flex justify-end mb-2">
                            {!isEditing ? (
                            <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 text-slate-700 rounded-lg text-xs font-semibold hover:bg-slate-200 transition-colors">
                                <Edit2 size={14} /> Sửa nội dung
                            </button>
                            ) : (
                            <button onClick={handleSaveEdit} className="flex items-center gap-2 px-3 py-1.5 bg-emerald-500 text-white rounded-lg text-xs font-semibold hover:bg-emerald-600 shadow-md transition-all">
                                <Save size={14} /> Lưu
                            </button>
                            )}
                        </div>
                        ...
                        <div className={`p-5 rounded-2xl border ${
                            ...
                            viewingStudent.aiReport.riskLevel === 'Trung bình' ? 'border-orange-200 bg-orange-50/50' : 'border-emerald-200 bg-emerald-50/50'
                        }`}>
                            <h3 className={`font-bold text-sm uppercase tracking-wide mb-3 ${
                                viewingStudent.aiReport.riskLevel === 'Cao' ? 'text-red-700' : 
                                viewingStudent.aiReport.riskLevel === 'Trung bình' ? 'text-orange-700' : 'text-emerald-700'
                            }`}>Đánh giá Tổng quan</h3>
                            {isEditing ? (
                            <textarea 
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-slate-400">
                        ...
                        <p>Chưa có báo cáo AI. Hãy nhấn nút "Tạo Báo Cáo" ở màn hình chính.</p>
                    </div>
                )
              )}
                  ...
                  <div>
                      ...
                      {viewingStudent.activeActions.length === 0 ? (
                          ...
                          <div className="space-y-6">
                              {viewingStudent.activeActions.map((action) => (
